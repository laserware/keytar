...
function stripMouseButtons(event: MouseEvent, chord: Chord): number {
  ...
  if (mouseButton === EventButton.Left) {
    if ((chord & MouseButton.Left) === MouseButton.Left) {
      strippedChord = strippedChord & ~MouseButton.Left;
    }
  }

  if (mouseButton === EventButton.Right) {
    if ((chord & MouseButton.Right) === MouseButton.Right) {
      strippedChord = strippedChord & ~MouseButton.Right;
    }
  }

  if (mouseButton === EventButton.Auxiliary) {
    if ((chord & MouseButton.Auxiliary) === MouseButton.Auxiliary) {
      strippedChord = strippedChord & ~MouseButton.Auxiliary;
    }
  }

  if (mouseButton === EventButton.BrowserBack) {
    if ((chord & MouseButton.BrowserBack) === MouseButton.BrowserBack) {
      strippedChord = strippedChord & ~EventButton.BrowserBack;
    }
  }

  if (mouseButton === EventButton.BrowserForward) {
    if ((chord & MouseButton.BrowserForward) === MouseButton.BrowserForward) {
      strippedChord = strippedChord & ~MouseButton.BrowserForward;
    }
  }
